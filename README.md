...
> **Karbon Emisyon Faktörü Kaynaklandırması:** Şebeke elektriği için kullanılan /usr/bin/bash.440	ext{ tCO}_2	ext{e/MWh}$ (/usr/bin/bash.440	ext{ kgCO}_2	ext{e/kWh}$) değeri, T.C. Enerji ve Tabii Kaynaklar Bakanlığı (ETKB) güncel elektrik tüketim emisyon faktörlerinde iletim bağlantılı tüketim (/usr/bin/bash.436	ext{ tCO}_2	ext{e/MWh}$) ve dağıtım bağlantılı tüketim (/usr/bin/bash.469	ext{ tCO}_2	ext{e/MWh}$) aralığında kurgulanmış **sentetik orta nokta varsayımıdır (synthetic midpoint assumption)**.
